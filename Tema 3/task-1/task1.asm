...
_sort:
	enter 0, 0
	sub 	esp, 20                 ; we need 5 local variables
...
inner_loop:
	...
	cmp   edx, DWORD 0
	jg    skip_current              ; if next != null, skip, it was already assigned
	...
	jge   skip_current
	mov   [ebp - 20], ebx           ; if all is ok move to current_min address of current node
skip_current:
	inc   ecx
	mov   [ebp - 8], ecx
	...
	cmp   eax, 0
	jg    existing_node_to_change   ; this means node_to_change != null
...
existing_node_to_change:
	...
	cmp   ecx, [ebp + 8]            ; compare with n
	jl    outer_loop
	add   esp, 20                   ; disalloc local variables
