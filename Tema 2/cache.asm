...
load:
    ...
    push    ebx
tags_loop:
    cmp     [ebx], edx   ; We are looping through the tags, in order to find one
    je      found_tag
    add     ebx, 4
    pop     esi
    push    esi
    push    ebx
    sub     ebx, esi
    cmp     ebx, 400     ; If we're running out of tags go to not found tag
    pop     ebx
    jl      tags_loop
    jmp     not_found
    
found_tag:
    mov     esi, ebx     ; Perform operations to get byte from cache
    pop     ebx
    pop     edx
    sub     esi, ebx
    shr     esi, 2
    mov     edi, 7       ; Mask to get last 3 bits
    and     edi, edx
    shl     esi, 3
    add     esi, edi
    mov     bl, BYTE [ecx + esi]
    ...
    jmp     end
not_found:          
    pop     ebx            ; Perform operations to add item in cache
    pop     edx 
    push    ecx
    mov     cl, BYTE [edx]
    mov     [eax], cl
    ...
    lea     ecx, [ecx + edi * 8]
cache_enter:
    push    ebx           ; actually add the 8 bytes in the cache
    mov     bl, BYTE [esi] 
    mov     [ecx], bl
    pop     ebx
    inc     esi
